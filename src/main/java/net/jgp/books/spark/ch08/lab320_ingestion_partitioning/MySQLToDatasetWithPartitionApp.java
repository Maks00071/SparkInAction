...
public class MySQLToDatasetWithPartitionApp {
    ...
    /**
     * {1} - столбец, по которому выполняется партицирование (распределение по разделам)
     * {2} - нижняя граница распределяемого диапазона записей (от ...)
     * {3} - верхняя граница распределяемого диапазона записей (до ...)
     * {4} - кол-во партиций (разделов)
     * {5} - потребление данных таблицы "film"
     */
    private void start() {
        ...
        props.put("partitionColumn", "film_id");        // {1}
        props.put("lowerBound", "1");                   // {2}
        props.put("upperBound", "1000");                // {3}
        props.put("numPartitions", "10");               // {4}
